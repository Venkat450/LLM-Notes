...
## Alternative Document Chain Workflows (with Example Scenarios)

| Document Chain Type         | What It Does                                                                                         | Example Scenario                                                                                         |
| --------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| **StuffDocumentsChain**     | Dumps all retrieved chunks into the prompt.                                                          | Best when you have 1-3 small chunks fitting in LLM's context window.                                     |
| **MapReduceDocumentsChain** | Summarizes each chunk (Map Phase), then combines those summaries into a final answer (Reduce Phase). | Ideal when documents are long, and you need the LLM to summarize and merge them.                         |
| **RefineDocumentsChain**    | Starts with an initial answer and refines it iteratively with each new chunk.                        | Useful when every new chunk should potentially update/refine the prior answer (step-by-step refinement). |

### Example Use Cases:

- **StuffDocumentsChain**: Quick Q&A over small docs (e.g., product FAQs).
- **MapReduceDocumentsChain**: Research summarization where you want per-paragraph summaries first.
- **RefineDocumentsChain**: Iterative reasoning tasks like building a detailed report that needs chunk-by-chunk refinement.
